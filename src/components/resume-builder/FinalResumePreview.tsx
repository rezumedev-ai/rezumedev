...
export function FinalResumePreview({
  ...
  useEffect(() => {
    const updateScale = () => {
      ...
      if (isMobile) {
        // Calculate scale based on viewport width for responsive sizing
        const viewportWidth = window.innerWidth;
        const pageWidth = 21 * 37.8; // 793px (A4 width)
        const padding = 32; // Total horizontal padding
        const scale = (viewportWidth - padding) / pageWidth;
        setResumeScale(Math.min(scale, 0.5)); // Max 0.5 for readability
      } else {
  ...
  return (
    <motion.div
      ref={containerRef}
      className="flex flex-col items-center min-h-screen bg-white py-4 relative overflow-x-hidden"
    ...
    >
      ...
      <div className="w-full flex justify-center">
        <motion.div
          id="resume-content"
          className="w-[21cm] min-h-[29.7cm] bg-white shadow-xl mb-6 relative overflow-hidden"
          style={{
            ...pageStyle,
